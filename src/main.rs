...
use x509_parser::{
    ...
    prelude::{FromDer, ParsedExtension, SignedCertificateTimestamp},
};
use std::collections::HashMap;
use sct::{verify_sct, Log as SctLog};
    ...
    for extension in x509cert.extensions() {
        if extension.oid.to_id_string() == "1.3.6.1.4.1.11129.2.4.2".to_string() {
            if let ParsedExtension::SCT(scts) = extension.parsed_extension() {
                println!("aaaaaaaaaaaaaaa ext raw {:?}", extension);
                ...
                for sct in scts {
                    ...
                    let length: usize = u16::from_be_bytes(extension.value[index..index+2].try_into().unwrap()) as usize;
                    println!("aaaaaaaaaaaaaaa length {length}");
                    index += 2;
                    let sct_raw: &[u8] = &extension.value[index..index+length];
                    index += length;

                    let tmp_ext_raw = &extension.value[6..(extension.value.len())];
